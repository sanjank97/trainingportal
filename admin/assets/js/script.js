...
var mobileInputs = document.getElementsByName('mobile');
for (var i = 0; i < mobileInputs.length; i++) {
  mobileInputs[i].addEventListener('keypress', function(event) {
    var keyCode = event.which ? event.which : event.keyCode;
    var isValid = /^[0-9]+$/.test(String.fromCharCode(keyCode));
    if (!isValid) {
...
var nameInputs = document.getElementsByName('name');
  ...
  for (var i = 0; i < nameInputs.length; i++) {
    nameInputs[i].addEventListener('keypress', function(event) {
      var keyCode = event.which ? event.which : event.keyCode;
      var inputValue = this.value + String.fromCharCode(keyCode);
